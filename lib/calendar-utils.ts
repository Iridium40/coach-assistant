...
export function generateGenericICSContent(event: CalendarEvent, attendeeEmail?: string, attendeeName?: string, organizerEmail?: string): string {
  ...
  const uid = event.uid || `${Date.now()}-${Math.random().toString(36).substr(2, 9)}@coachassistanthub.com`
  ...
  if (organizerEmail) {
    ...
    lines.push(`ORGANIZER;CN=Coach Assistant Hub:mailto:noreply@coachassistanthub.com`)
  }
...
export function generateICSContent(call: ZoomCall): string {
  ...
  const uid = `${call.id}@coachassistanthub.com`
  const now = formatDate(new Date())
  ...
  const icsContent = [
    ...
    'STATUS:CONFIRMED',
    `ORGANIZER;CN=Coach Assistant Hub:mailto:noreply@coachassistanthub.com`,
    'END:VEVENT',
    'END:VCALENDAR'
  ].filter(Boolean).join('\r\n')
