...
✨ Let us know when your box arrives because that's when the magic really begins!`
  ...
  return (
    <div className="max-h-[85vh] overflow-y-auto">
      {/* Tool Header */}
      <div className="bg-gradient-to-r from-[hsl(var(--optavia-green))] to-[#00c760] text-white shadow-lg shadow-green-500/30 mb-4 -mx-6 -mt-4 px-6 py-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-2">
            <Button
              ...
              size="sm"
              onClick={resetAll}
              className="bg-white/10 border-white/50 text-white hover:bg-white/20 h-8"
            >
            ...
            <Button
              ...
              className="bg-white text-[hsl(var(--optavia-green))] hover:bg-gray-50 border-white h-8"
            >
      ...
      <div className="mb-4">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabId)}>
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="tips" className="text-xs">9 Tips</TabsTrigger>
            <TabsTrigger value="followup" className="text-xs">Follow-Up</TabsTrigger>
            <TabsTrigger value="quick" className="text-xs">Quick Copy</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>

      {/* Client Info Card */}
      <Card className="mb-4 bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-[hsl(var(--optavia-green))]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input
                ...
                className="border-green-200 focus:border-[hsl(var(--optavia-green))] focus:ring-[hsl(var(--optavia-green))] h-9"
              />
            </div>
            <div className="space-y-1">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabId)}>
        ...
        <TabsContent value="welcome" className="space-y-4 mt-4">
          <Card className="bg-blue-50 border-blue-200">
            <CardContent className="pt-4">
              <div className="flex items-center gap-2 font-bold text-blue-800 mb-2 text-sm">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">📱 Welcome & 9 Tips Text</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="relative bg-slate-50 border border-slate-200 rounded-xl p-4">
                ...
                <Button
                  ...
                  className="absolute top-3 right-3 bg-[hsl(var(--optavia-green))] hover:bg-green-700 h-7"
                >
                  <Copy className="h-3 w-3 mr-1" />
                  Copy
                </Button>
                <div className="whitespace-pre-wrap text-xs leading-relaxed text-slate-700 pr-20 max-h-96 overflow-y-auto">
                  {welcomeText}
                </div>
              </div>
            </CardContent>
        ...
        <TabsContent value="tips" className="space-y-4 mt-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-xs font-bold text-slate-500">Client Prep Progress</CardTitle>
            </CardHeader>
            <CardContent>
              <Progress value={tipProgressPercent} className="h-2 mb-2" />
            ...
            {tips.map((tip) => (
              <Card key={tip.id} className="overflow-hidden">
                <div className="bg-slate-50 border-b border-slate-200 p-3 flex items-center gap-2">
                  <div className="w-8 h-8 bg-gradient-to-br from-[hsl(var(--optavia-green))] to-[#00c760] text-white rounded-lg flex items-center justify-center font-extrabold text-sm flex-shrink-0">
                    {tip.id}
                  </div>
                  <div className="font-bold text-slate-800 text-sm flex-1">{tip.title}</div>
                  <div
                    className={`w-5 h-5 border-2 rounded-md cursor-pointer flex items-center justify-center transition-all ${
                  ...
                  >
                    {checkedTips.has(tip.id) && <Check className="h-3 w-3 text-white" />}
                  </div>
                </div>
                <CardContent className="p-3 space-y-2">
                  <p className="text-xs text-slate-700">{tip.what}</p>
                  <div className="bg-purple-50 border-l-4 border-purple-500 p-2 rounded-r-lg">
                    ...
                    <p className="text-xs text-purple-800">{tip.why}</p>
                  </div>
                  {tip.link && (
                    <div>
                      <a
                        ...
                        className="inline-flex items-center gap-1 text-xs text-[hsl(var(--optavia-green))] font-semibold hover:underline"
                      >
                        🔗 {tip.linkText}
                      </a>
                    </div>
                  )}
                  <div className="bg-green-50 border-l-4 border-[hsl(var(--optavia-green))] p-2 rounded-r-lg">
                    ...
                    <p className="text-xs text-green-800 italic">{tip.script}</p>
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <TabsContent value="followup" className="space-y-4 mt-4">
          ...
          <div className="relative pl-6 max-h-[60vh] overflow-y-auto">
            <div className="absolute left-2 top-0 bottom-0 w-0.5 bg-slate-200" />
            {timelineItems.map((item, idx) => (
              <div key={idx} className="relative mb-4 bg-white border border-slate-200 rounded-xl overflow-hidden">
                <div className="absolute -left-5 top-4 w-3 h-3 bg-white border-3 border-[hsl(var(--optavia-green))] rounded-full" />
                <div className="bg-slate-50 border-b border-slate-200 p-3 flex items-center justify-between">
                  <div className="font-bold text-slate-800 text-sm flex items-center gap-2">
                    <span className="bg-gradient-to-r from-[hsl(var(--optavia-green))] to-[#00c760] text-white px-2 py-0.5 rounded-full text-xs font-bold">
                      {item.badge}
                    </span>
                    {item.title}
                  </div>
                ...
                <div className="p-3 space-y-2">
                  {item.tasks.map((task, taskIdx) => {
                    ...
                    return (
                      <div key={taskIdx} className="flex items-start gap-2 pb-2 border-b border-slate-100 last:border-0">
                        <div
                          className={`w-4 h-4 border-2 rounded cursor-pointer flex items-center justify-center flex-shrink-0 mt-0.5 transition-all ${
                        ...
                        >
                          {checkedTasks.has(currentTaskIndex) && <Check className="h-2.5 w-2.5 text-white" />}
                        </div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {messages.map((msg, idx) => (
                      <button
                        ...
                        className="text-left p-3 bg-slate-50 border border-slate-200 rounded-lg hover:border-[hsl(var(--optavia-green))] hover:bg-green-50 transition-all"
                      >
                        <div className="font-semibold text-slate-800 text-xs mb-1">{msg.title}</div>
                        <div className="text-xs text-slate-500 line-clamp-2">{msg.text}</div>
                      </button>
                    ))}
                  </div>
                </CardContent>
